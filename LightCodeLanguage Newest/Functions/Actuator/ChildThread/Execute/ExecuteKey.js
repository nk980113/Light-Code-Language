...
function executeKey (chunk, complexType) {
  if (chunk.returnData.type === 'object') {
    ...
    for (let run = 1; run < allKey.length; run++) {
      if (container2 === undefined || container2.type !== 'object') throwError(chunk, { error: true, content: `無法從一個 <${(container2 === undefined) ? '無' : typesName[container2.type]}> 中使用 <鑰> 來讀取值`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{分析器}', line: complexType.line }]})
    ...
    throwError(chunk, { error: true, content: `無法從一個 <${typesName[chunk.returnData.type]}> 中使用 <鑰> 來讀取值`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{分析器}', line: complexType.line }]})
  }
}
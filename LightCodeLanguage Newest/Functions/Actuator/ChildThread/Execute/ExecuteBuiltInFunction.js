  ...
  if (complexType.value === '變數') {
    if (chunk.returnedData === undefined) {
      if (chunk.complexTypes[chunk.executiveData.row+1] === undefined) {
        throwError(chunk, { error: true, content: `後面必須為一個 <容器>`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
      } else {
        ...
        if (container === undefined) {
          ...
          throwError(chunk, { error: true, content: `已經有名為 ${chunk.complexTypes[chunk.executiveData.row+1].value} 的容器`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
        }
      }
    } else {
    ...
    if (chunk.complexTypes[chunk.executiveData.row+1].type !== 'builtInFunction' && chunk.complexTypes[chunk.executiveData.row+1].type !== 'container') throwError(chunk, { error: true, content: `<內建功能> 唯讀 的後面只能為一個 <內建功能> 或 <容器>(在物件裡)`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
    chunk.executiveData.mode = 'readOnly'
  } else if (complexType.value === '等待') {
    if (!(chunk.complexTypes[chunk.executiveData.row+1].type === 'builtInFunction' && chunk.complexTypes[chunk.executiveData.row+1].value === '函數') && chunk.complexTypes[chunk.executiveData.row+1].type !== 'container') throwError(chunk, { error: true, content: `<內建功能> 異同步 的後面只能為一個 <內建功能> 函數 或 <容器>()`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
    chunk.executiveData.mode = 'async'
  } else if (complexType.value === '函數') {
    if (chunk.complexTypes[chunk.executiveData.row+1] === undefined) throwError(chunk, { error: true, content: `<內建功能> 函數 的後面只能為一個 <容器> 或 <參數列>`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
    if (chunk.complexTypes[chunk.executiveData.row+1].type === 'container') {
      if (chunk.complexTypes[chunk.executiveData.row+2] === undefined || chunk.complexTypes[chunk.executiveData.row+2].type !== 'parameters' || chunk.complexTypes[chunk.executiveData.row+3].type !== 'chunk') {
        throwError(chunk, { error: true, content: `創建函數的格式錯誤 (必須為 "函數 <容器> <參數列> <區塊>" 或 "函數 <參數列> <區塊>")`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
      } else {
        let container = getContainer(chunk.layer, chunk.complexTypes[chunk.executiveData.row+1].value)
        if (container === undefined) {
          let parameters = []
          if (parameters.length > 0) {
            for (let run = 0; run < chunk.complexTypes[chunk.executiveData.row+2].value.length; run++) {
              if (chunk.complexTypes[chunk.executiveData.row+2].value[run][0].type !== 'container' || chunk.complexTypes[chunk.executiveData.row+2].value[run].length > 1) {
                throwError(chunk, { error: true, content: `函數的參數列的第 ${run} 項只能為一個 <容器> (容器名)`, start: complexType.start, end: complexType.end, path: [{ func: (chunk.name === '全局') ? undefined : '{執行器}', line: complexType.line }]})
              } else {
